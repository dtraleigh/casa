...
- **HouseholdFact model** — shared knowledge, admin-curated (auto-learning deferred to Phase 3b)
...
**HouseholdFact** — shared knowledge about the household. Every fact visible to every user's conversations. Phase 1 is admin-curated only; Phase 3b adds auto-learning from conversations.
...
- **One HouseholdFact per row, not a text blob.** Individual facts are easier to add, edit, and delete. Also makes future auto-learning (Phase 3b) much cleaner — each learned fact becomes its own row with source attribution.
- **`source_user` on HouseholdFact** is unused in Phase 1 but present in the schema so no migration is needed when Phase 3b arrives.
...
- **No cross-database foreign keys — user references are decoupled integers.**
  ...
  - `HouseholdFact.source_user` (FK) → `source_user_id` (`IntegerField`, null/blank)
    + `source_username` (`CharField`). Unused in Phase 1; present for Phase 3b.
...
> **Status: ✅ Complete — 2026-07-26.** Conversations now persist to the `ai_lab`
> database, resume by UUID, and are browsable/deletable via a History page, where
> they can also be favorited (pinned to a Favorites section on top). 41 tests
> pass. See **As-built notes** below for where the implementation deviated from
> this plan (server-owned history, decoupled owner, async title endpoint,
> per-user privacy, delete + favorites).
...
- Delete a conversation from the History page (cascades to its messages)
- **Favorite conversations** — a per-conversation star toggle on the History page pins favorites to a Favorites section on top (both sections ordered by most-recent-activity)
...
What shipped differs from the sketch above in a few deliberate places. Recorded here so
Phase 3+ builds on reality.

- **History moved from browser to server.** Phase 1 was stateless — the browser held the
  authoritative message array and re-POSTed the full history every turn. Phase 2 makes the
  **server the source of truth**: `POST /mycroft/send/` now takes `{conversation_id, content}`,
  reconstructs the sliding window (`MYCROFT_HISTORY_WINDOW`, default 20) from the DB, streams,
  and persists both turns itself. The browser tracks only the conversation id. This was the
  central rework and rewrote both `send_message` and the template JS.

- **`Conversation` has a decoupled owner — the draft model omitted it.** Conversations are
  private per-user, so `Conversation` carries `user_id` (`IntegerField`, indexed) + `username`,
  the same cross-DB-safe pattern as `HouseholdFact`/`UserContext` (no FK to `auth.User`, which
  lives in `default`). Every resume/history/title/delete path filters on `user_id` and 404s a
...
- **`mycroft/memory.py` was introduced now, not in Phase 3.** The plan slots it for Phase 3,
  ...
  thin. It's the seam Phase 4 (tool messages), Phase 3 (embeddings, retrieval), and Phase 3b
  (auto-learning "after each exchange") attach to.

- **Titles are generated by a separate non-streaming endpoint.** There's no background-task
  infra (sync gunicorn), so after the first exchange the browser calls
  `POST /mycroft/conversation/<uuid>/title/`, which does one non-streaming Ollama call
  (`client.complete_chat`) and saves the title. It's off the streaming worker; on failure the
  title stays blank and both the header and History fall back to a first-message snippet
  (`Conversation.display_title()`).

- **Delete + read-only admin.** The History page includes a per-conversation delete (with a
  confirm), cascading to messages. `Conversation`/`Message` are registered in admin with a
  read-only message inline for transcript review (audit records, not hand-edited).

- **Persistence happens inside the streaming generator.** The assistant turn is written after
  the token loop (partial text saved if the stream errors mid-way — honours the
  transcript-visible principle), and `updated_at` is bumped explicitly (`touch`) so a related
  `Message` insert alone wouldn't reorder history. DB writes in the generator run before
  `request_finished`, so this is safe.

- **Favorites (added after the initial Phase 2 build).** A `Conversation.is_favorite` flag
  (indexed) drives a star toggle on each History row (`POST /conversation/<uuid>/favorite/`,
  owner-scoped). Favorited conversations render in a pinned Favorites section above the rest,
  each ordered by `updated_at` desc; the star updates and the row moves between sections live
  (no reload). Toggling favorite uses `save(update_fields=['is_favorite'])` so it deliberately
  does **not** bump `updated_at` — favoriting isn't activity. Favorites are unlimited and
  excluded from the 50-row cap on the main list, so pinning also guarantees a conversation stays
  reachable past that cap.

- **Bootstrap Icons added for the star.** The project only loaded Bootstrap core (no icon font),
  so `bootstrap-icons@1.11.3` is now loaded from the same CDN in `templates/includes/head.html`
  (`bi-star` / `bi-star-fill`). Note: the `extra_css` and `title` blocks live inside the
  `{% templatetag openblock %} include {% templatetag closeblock %}`d `head.html`, so they can't
  be overridden by child templates — page-scoped CSS rides in the content block instead, and
  per-page `<title>` overrides currently don't take effect (latent, pre-existing).

---

## Phase 3: Semantic memory + knowledge base

> **Status: ✅ Core shipped — 2026-08-01.** Message embeddings, the `Knowledge`
> model, and semantic retrieval into the system prompt are built and covered by
> unit/integration tests (74 pass, retrieval mocked). **Still open:** a live
> smoke test with the real embed model and a retrieval-tuning pass (the
> distance/top-K knobs were picked blind). **Auto-learning of HouseholdFacts was
> split out into [Phase 3b](#phase-3b-auto-learning-of-householdfacts) — it needs
> its own scoping.** See **As-built notes** for where the build deviated.

**Goal:** Mycroft can recall things you told him weeks ago, and can be pre-loaded with facts about you and your world.

**What shipped (core):**

- `embedding` column on `Message` (`pgvector.django.VectorField`, nullable)
- Every message (user + assistant) embedded with `nomic-embed-text` at write time
- New `Knowledge` model for curated facts (not conversation-derived), embedded on save
- Retrieval layer in `mycroft/memory.py`: given a user message, find top-K semantically relevant past messages and Knowledge entries
- Retrieved context injected into the system prompt for that turn
- Admin interface for curating the Knowledge table (Django admin)

**Data model additions (as built):**
...
class Knowledge(models.Model):
    ...
    content = models.TextField()
    embedding = VectorField(dimensions=768, null=True, blank=True)  # embedded on save
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
```
...
**Definition of done (validate live — pending):**

- Have a long conversation about a topic. Come back later, mention the topic obliquely, and Mycroft brings up relevant details from the earlier exchange.
...
What shipped differs from the sketch above in a few deliberate places.

- **Embed every message, best-effort, inline.** Both user and assistant turns are
  embedded (resolving the open question below in favour of full coverage). The user
  turn is embedded synchronously in `send_message` *before* the chat call, and that
  same vector is reused as the retrieval query — no second embed for the query. If an
  embed call fails (Ollama down, model missing), `memory.embed_message` logs and
  swallows it and the row is saved with a null embedding (unretrievable) rather than
  breaking the chat. There's no background-task infra (sync gunicorn), so embedding
  rides on the request path — same constraint that made Phase 2's title generation a
  separate endpoint.

- **pgvector extension is provisioned manually, not by the migration.** `ai_lab_user`
  isn't a Postgres superuser, so `CREATE EXTENSION vector` is a one-time superuser step
  on the `ai_lab` DB (and on `template1`, so the ephemeral `test_ai_lab` inherits it).
  Migration `0006` adds the `VectorField` columns only — **no** `VectorExtension()`
  operation — so `migrate --database=ai_lab` runs as the unprivileged app user.

- **Retrieval is privacy-scoped and de-duplicated.** Past-message recall is filtered to
  the requesting user's own messages and **excludes the current conversation** (its
  recent turns are already in the sliding window). Both messages and Knowledge are
  ranked by pgvector `CosineDistance` and dropped beyond a distance ceiling so weak
  matches don't pollute the prompt. `Knowledge` is global (shared, curated).

- **Retrieval is tunable via settings/`.env`** without a redeploy: `OLLAMA_EMBED_MODEL`,
  `MYCROFT_RETRIEVAL_MESSAGES` (5), `MYCROFT_RETRIEVAL_KNOWLEDGE` (3),
  `MYCROFT_RETRIEVAL_MAX_DISTANCE` (0.55). The distance/K defaults are provisional —
  the live tuning pass exists to set them.

- **Auto-learning deferred to Phase 3b** (see below) — the core recall loop ships and
  gets validated first.

**Open questions — resolved / pending:**

- ~~Embed *every* message vs. user-only?~~ **Resolved: every message** (user + assistant).
- **How aggressive should recall be?** Still open — answerable only with the live
  tuning pass. Lower `MYCROFT_RETRIEVAL_MAX_DISTANCE` if Mycroft over-references old
  chatter; raise it if he feels forgetful.

---

## Phase 3b: Auto-learning of HouseholdFacts

> **Status: ✅ Shipped — 2026-08-21.** After each exchange the browser fires an
> off-band `/learn/` endpoint that asks Mycroft to extract durable household facts
> and writes new ones as `source='learned'` HouseholdFacts. Covered by unit +
> integration tests (`complete_chat` mocked). **Deviations from the original
> sketch below** — no admin approval gate, no auto-learning management-command;
> see **As-built notes**.

**Goal:** Mycroft passively grows the shared household knowledge base from
conversations, instead of every fact being hand-entered in admin.

**What ships:**

- **Auto-learning of HouseholdFacts** — a step runs after each conversation exchange,
  evaluates whether a durable household fact was surfaced, and creates a `HouseholdFact`
  row with `source='learned'` and `source_user_id`/`source_username` set (the schema
  already carries these from Phase 1). Admin reviews learned facts and can approve, edit,
  or delete them.

### Auto-learning design notes

The reason this is carved out: it's the hard part and deserves its own scoping. Key concerns:
...
- **Where it runs.** No background-task infra (sync gunicorn), so the extraction call needs a home — likely a separate off-band endpoint like Phase 2's title generation, not the streaming path.

**Definition of done:**

- Mention a durable fact in conversation ("we got a dog named Biscuit"). After the
  exchange, a `HouseholdFact` with `source='learned'` appears in admin.
- A transient remark ("I'm tired today") does *not* get promoted to a household fact.

### As-built notes (deviations from this plan)

What shipped differs from the sketch above in a few deliberate places.

- **Learned facts go live immediately — no approval gate.** The plan implied a
  review-before-use workflow; instead learned facts feed the system prompt as soon
  as they're written (`build_system_prompt` already loads every `HouseholdFact`).
  Admin review is *cleanup after the fact* (edit/delete/prune), not pre-approval —
  chosen for simplicity. There is therefore **no new field and no migration**; the
  Phase 1 `source` / `source_user_id` / `source_username` columns carry it.

- **Off-band per-exchange endpoint, mirroring title generation.** `POST
  /mycroft/conversation/<uuid>/learn/` (owner-scoped) runs extraction via the
  non-streaming `complete_chat` and is fired fire-and-forget by the browser after
  *every* completed reply (guarded on a non-truncated response). It's off the
  streaming worker; a failure is logged and swallowed (`memory.learn_from_exchange`
  returns `[]`) so the chat is never affected — same best-effort discipline as
  Phase 3 embedding. No background-task infra, so this rides the request path like
  Phase 2's title endpoint.

- **Conservative extraction into a JSON array, parsed defensively.**
  `build_extraction_prompt` asks Mycroft to return only durable facts (names,
  pets, vehicles, routines…), excluding transient/confidential material, as a bare
  JSON array — `[]` when nothing qualifies. `memory._parse_fact_list` tolerates
  code fences / surrounding prose (slices first `[` … last `]`), drops non-strings,
  and caps at 5 facts per exchange so a runaway reply can't flood the table.

- **Exact-text dedup only (no embedding column on `HouseholdFact`).** A candidate
  is skipped if its *normalized* text (lowercased, whitespace-collapsed, trailing
  period stripped) already exists. Near-duplicates and contradictions are left to
  admin pruning — surfaced by `source`/`source_username` in the changelist and a
  `source='learned'` filter. Embedding-based dedup + contradiction detection is a
  noted follow-up.

- **No management command.** Extraction is purely event-driven off the endpoint;
  the batch-command option was not built.
...
- **Auto-learning extraction quality** — Phase 3b shipped; tune the extraction prompt / dedup during live validation. Embedding-based dedup + contradiction detection remain a follow-up.
...
| Phase | Status |
|-------|--------|
| 1 — Basic chat + personality | ✅ Complete (2026-07-19) |
| 2 — Persistent conversations | ✅ Complete (2026-07-26) |
| 3 — Semantic memory + knowledge base (core) | ✅ Code shipped (2026-08-01); **live validation still open** |
| 3b — Auto-learning of HouseholdFacts | ✅ Code shipped (2026-08-21), 14 tests pass; **live validation still open** |
| 4 — Tool calling (weather) | 🔲 Not started |
| 5 — Voice I/O | 🔲 Not started, unscoped |

**Next actions, in order:**

1. **Live-validate Phase 3b** (code is in, tests green). In a real chat say
   "we got a dog named Biscuit" → confirm a `source='learned'` HouseholdFact
   appears in admin and is referenced on a later turn; a transient remark
   ("I'm tired today") should create nothing. Tune the extraction prompt /
   `_MAX_LEARNED_PER_EXCHANGE` if it over- or under-harvests.
2. **Live-validate + tune Phase 3 core** — run the DoD smoke tests and set the
   retrieval knobs (`MYCROFT_RETRIEVAL_MAX_DISTANCE`, `_MESSAGES`, `_KNOWLEDGE`),
   which were picked blind. See the Phase 3 "Definition of done (validate live)".
3. **Build Phase 4 — tool calling (weather)** — the next clean feature build;
   the `mycroft/tools.py` seam and `role='tool'` message plumbing are ready.

Uncommitted as of this update: Phase 3b (this build) plus the earlier
HouseholdFact admin help-text change and migration `0007`.
